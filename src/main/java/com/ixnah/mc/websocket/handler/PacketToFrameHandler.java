...
public class PacketToFrameHandler extends MessageToMessageEncoder<ByteBuf> {
    ...
    protected void encode(ChannelHandlerContext ctx, ByteBuf msg, List<Object> out) {
        ...
        if (length < maxFrameSize) {
            ...
            out.add(new BinaryWebSocketFrame(false, 0, msg.readSlice(maxFrameSize).retain()));
            for (int i = 1, size = (int) Math.ceil(length / (double) maxFrameSize) - 1; i < size; i++) {
                out.add(new ContinuationWebSocketFrame(false, 0, msg.readSlice(maxFrameSize).retain()));
            }
            out.add(new ContinuationWebSocketFrame(true, 0, msg.readSlice(msg.readableBytes()).retain()));
        }
    }
}
